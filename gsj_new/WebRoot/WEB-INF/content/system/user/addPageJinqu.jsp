...
.listBtn {
	...
	<body>
		<div id="scrollContent">
			<s:form action="user!save.action?fromUrl=jingqu" id="form1">
				<div class="box2" panelWidth="100%" panelTitle="景区用户信息"
					...
					<table width="95%" class="tableStyle">
						...
						<tr>
							...
							<td>
								<s:textfield name="user.begintime"
						...
						<tr>
							...
							<td>
								 <select name="user.user.id" >
								 	<c:forEach var="item" items="${users}">
								 		<option value="${item.id }" <c:if test="${item.id==user.user.id}">selected</c:if>>
								 		${item.realname }
								 		</option>
								 	</c:forEach>
								 </select>
