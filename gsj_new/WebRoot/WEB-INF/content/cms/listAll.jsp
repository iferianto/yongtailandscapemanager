...
<tr>
	<td height="30px;" style="border-bottom:1px dashed #eee;font-size:13px;">
	<span class="riqi" style="padding-right:10px;"><fmt:formatDate value="${item.pubdate}" /></span>
								<img src="${ctx }/cms/images/_icon02.gif"/>  <a target="_blank" title="${item.title }"
										href="${ctx}/cms!detail.action?seqs=${item.id}"> <c:if
											test="${fn:length(item.title) > 40}">
											<c:out value="${fn:substring(item.title, 0, 40)}..." />
										</c:if> <c:if test="${fn:length(item.title) <= 40}">
												${item.title }</c:if></a>
	</td>
</tr>
</c:forEach>
