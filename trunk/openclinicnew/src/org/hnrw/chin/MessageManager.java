...
/**
 * Created by IntelliJ IDEA. User: Frank Date: 23-mei-2008 Time: 20:26:31 To
...
public class MessageManager {
	...
	public static boolean readMessages() {
		...
		Connection oc_conn = MedwanQuery.getInstance()
				.getOpenclinicConnection();
		try {
	...
	public static boolean receiveMessages() {
		boolean bSuccess = false;
		String sDoc = MedwanQuery.getInstance().getConfigString(
				"templateSource")
				+ "healthnet.xml";
		...
		try {
			...
			while (!ftp.isConnected() && retries < 1) {
				retries++;
				try {
					if (Internet.isReachable(server.attributeValue("host"),
							Integer.parseInt(server
									.attributeValue("port", "21")), 1000)) {
						ftp.connect(server.attributeValue("host"), Integer
								.parseInt(server.attributeValue("port", "21")));
						break;
					}
				} catch (Exception e) {
				}
			...
			if (ftp.isConnected()) {
				ftp.login(server.attributeValue("user"), server
						.attributeValue("password"));
				// Nu moeten we alle berichtjes downloaden die op de server
				// beschikbaar zijn
				FTPFile[] files = ftp.listFiles();
				for (FTPFile file : files) {

					String filename = file.getName();
					ftp.downloadFile(filename, (integrator.attributeValue(
							"directory", "c:/hn/received")
							+ "/" + filename).replaceAll("//", "/"));
					Document msg = reader.read((integrator.attributeValue(
							"directory", "c:/hn/received")
							+ "/" + filename).replaceAll("//", "/"));
				...
				try {
					if (!initialized) {
						...
						while (!ftp.isConnected() && retries < 1) {
							retries++;
							try {
								if (Internet.isReachable(server
										.attributeValue("host"), Integer
										.parseInt(server.attributeValue("port",
												"21")), 1000)) {
									ftp.connect(server.attributeValue("host"),
											Integer.parseInt(server
													.attributeValue("port",
															"21")));
									break;
					...
					if (initialized) {
						...
						ftp.uploadFile(filename, client.attributeValue("id")
								+ "."
								+ filename.replaceAll(
										storage.attributeValue("directory",
												"c:/hn/messages"), "")
										.replaceAll("/", ""));
	...
	public static boolean storeMessage(Element message) {
		...
		try {
			...
			String filename = storage.attributeValue("directory",
					"c:/hn/messages")
					+ "/" + message.attributeValue("created") + ".hn.msg";
			...
			PreparedStatement ps = oc_conn.prepareStatement(sQuery);
			ps.setString(1, filename);
			ps.setTimestamp(2, new Timestamp(new SimpleDateFormat(
					"yyyyMMddHHmmssSSS").parse(
					message.attributeValue("created")).getTime()));
			ps.execute();
			ps.close();
			bSuccess = true;
		} catch (Exception e) {
