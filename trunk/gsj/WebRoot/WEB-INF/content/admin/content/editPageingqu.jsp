...
$().ready(function() {
	...
	<body>
		<div id="scrollContent">
			<s:form action="content!savejingqu.action" id="form1"enctype="multipart/form-data">
				<div class="box2" panelWidth="100%" panelTitle="信息修改"
					...
					<s:hidden name="content.audit"  value="0" />
					<table width="95%" class="tableStyle">
						<tr>
							...
							<td width="85%" colspan="3">
								<s:textfield name="content.title" maxlength="200"
						...
						<tr>
							...
							<td>
								  <s:hidden name="content.type.id"></s:hidden>
								 <s:text name="content.typename"></s:text>
							</td>
							<td align="right">
								是否置顶：
							</td>
						...
						<tr>
													...
													<td width="38%" colspan="3">
														<input type=button onclick="add();" class="button" value="添加附件">
															<div id="files">
															 <c:forEach items="${photos}" var="item">
															 <li ><a href="${ctx }${item.imgurl}" target="_blank">${item.imgurl }点击查看</a><input type=button class='button' value='删除' onclick="remove1('${item.id}',this)"></li>
															 </c:forEach>
																<script>
																var phc=${photos_size};
				...
				function remove1(sid,obj)
				{
					if(confirm('你确定要删除吗？')){
						$.get("${ctx}/admin/content!deletephoto.action", { id:sid },  
						function(data){  
						 if(data=='success'){
