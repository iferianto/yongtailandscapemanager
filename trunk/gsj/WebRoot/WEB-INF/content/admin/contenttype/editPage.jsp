		...
		<div id="scrollContent">
			<s:form action="content-type!save.action" id="form1">
				<div class="box2" panelWidth="100%" panelTitle="表单填写"
					...
					<s:hidden name="contentType.id"></s:hidden>
...
<option value="">请选择</option><c:forEach items="${types}" var="item"><option value="${item.id }" <c:if test="${item.id==contentType.parent.id}">selected</c:if>>${item.name }</option></c:forEach>
</select>
							</td>
							<td>
						 		类型: 
