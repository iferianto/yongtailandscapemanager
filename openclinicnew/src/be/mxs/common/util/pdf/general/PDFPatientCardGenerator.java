...
/**
 * User: stijn smets Date: 21-nov-2006
 */
public class PDFPatientCardGenerator extends PDFOfficialBasic {
	...
	public ByteArrayOutputStream generatePDFDocumentBytes(
			final HttpServletRequest req, AdminPerson person) throws Exception {
		...
		this.url = this.getClass().getClassLoader().getResource("/").getPath();
		...
		try {
			...
			Rectangle rectangle = new Rectangle(0, 0, new Float(
					MedwanQuery.getInstance().getConfigInt("patientCardWidth",
							540) * 72 / 254).floatValue(), new Float(
					MedwanQuery.getInstance().getConfigInt("patientCardHeight",
							860) * 72 / 254).floatValue());
	...
	protected void printPatientCard(AdminPerson person) {
		try {
			...
			// Logo
			Image image = Image.getInstance(url	+ "_img/logo_patientcard.gif");
			...
			cell = createLabel(MedwanQuery.getInstance().getLabel("web",
					"cardarchivecode", user.person.language), 6, 1, Font.ITALIC);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
			cell.setPadding(0);
			subTable.addCell(cell);
			cell = createLabel(person.getID("archiveFileCode").toUpperCase(),
					10, 1, Font.BOLD);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
			cell.setPadding(0);
			subTable.addCell(cell);
			...
			table.addCell(cell);
			cell = createLabel(MedwanQuery.getInstance().getLabel("web",
					"cardname", user.person.language), 6, 4, Font.ITALIC);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
			table.addCell(cell);
			cell = createLabel(person.firstname + " " + person.lastname, 10, 4,
					Font.BOLD);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
			table.addCell(cell);
			...
			cell = createLabel(MedwanQuery.getInstance().getLabel("web",
					"carddateofbirth", user.person.language), 6, 2, Font.ITALIC);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
			table.addCell(cell);
			if (MedwanQuery.getInstance().getConfigInt("patientCardModel", 1) == 2) {
				cell = createLabel(MedwanQuery.getInstance().getLabel("web",
						"cardinsurancenumber", user.person.language), 6, 1,
						Font.ITALIC);
				cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
				table.addCell(cell);
				cell = createLabel(MedwanQuery.getInstance().getLabel("web",
						"cardgender", user.person.language), 6, 1, Font.ITALIC);
				cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
				table.addCell(cell);
			} else {
				cell = createLabel(MedwanQuery.getInstance().getLabel("web",
						"cardgender", user.person.language), 6, 2, Font.ITALIC);
				cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
				table.addCell(cell);
			}
			...
			cell = createLabel(MedwanQuery.getInstance().getLabel("web",
					"cardhospitalref", user.person.language), 6, 4, Font.NORMAL);
			cell.setBorder(PdfPCell.TOP);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
			table.addCell(cell);

			doc.add(table);
			doc.setJavaScript_onLoad(MedwanQuery.getInstance().getConfigString(
					"cardJavaScriptOnLoad", "document.print();"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	protected PdfPCell createUnderlinedCell(String value, int colspan) {
		cell = new PdfPCell(new Paragraph(value, FontFactory.getFont(
				FontFactory.HELVETICA, 7, Font.UNDERLINE))); // underlined
	...
	protected PdfPCell createTitle(String msg, int colspan) {
		cell = new PdfPCell(new Paragraph(msg, FontFactory.getFont(
	...
	// --- CREATE TITLE
	// ----------------------------------------------------------------------------
	protected PdfPCell createLabel(String msg, int fontsize, int colspan,
			int style) {
		cell = new PdfPCell(new Paragraph(msg, FontFactory.getFont(
	...
	// ------------------------------------------------------------------
	protected PdfPCell createBorderlessCell(String value, int height,
			int colspan) {
		cell = new PdfPCell(new Paragraph(value, FontFactory.getFont(
	...
	protected PdfPCell createItemNameCell(String itemName, int colspan) {
		cell = new PdfPCell(new Paragraph(itemName, FontFactory.getFont(
				FontFactory.HELVETICA, 7, Font.NORMAL))); // no uppercase
		cell.setColspan(colspan);
